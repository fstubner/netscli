...
| `output.rs` + `output/` | `--json`/`--yaml`/`--csv` output-format selection; `output/csv.rs` renders CSV from the same serialized result |
...
- Output formats: CLI subcommands support `--json` and `--yaml` flags; list-shaped ones also take `--csv`
